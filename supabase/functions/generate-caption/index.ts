...
serve(async (req) => {
  ...
  try {
    ...
    if (currentCount >= 20) {
      return new Response(
        JSON.stringify({ error: 'AI caption limit reached (20/20 today)' }), 
    ...
    return new Response(
      JSON.stringify({ error: 'Caption generation unavailable. Try again?' }),
...
async function generateCaption(params: any, apiKey: string): Promise<string> {
  ...
  const styleAnalysis = userStyle.length > 0 
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: 'gpt-3.5-turbo',
      messages: [
        { role: 'system', content: 'You are a witty sports betting caption writer. Keep it short and punchy.' },
