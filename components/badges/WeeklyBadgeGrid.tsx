...
import { WEEKLY_BADGES, getBadgeExpirationInfo } from '@/data/weeklyBadges';
...
export const WeeklyBadgeGrid: React.FC<WeeklyBadgeGridProps> = ({
  badges,
  earnedDate = new Date(),
}) => {
    ...
    return (
      ...
      >
        <Text fontSize="$3" color={Colors.text.secondary}>
          No badges earned this week yet
        </Text>
        <Text fontSize="$2" color={Colors.text.tertiary} marginTop="$1">
  ...
  const { daysRemaining, hoursRemaining } = getBadgeExpirationInfo(earnedDate);

  return (
    <Stack space="$2">
      {/* Expiration info */}
      <View flexDirection="row" justifyContent="space-between" alignItems="center">
        <Text fontSize="$2" color={Colors.text.secondary} fontWeight="600">
          Weekly Badges
        </Text>
        <Text fontSize="$1" color={Colors.text.tertiary}>
          {daysRemaining > 0 ? `Expires in ${daysRemaining}d` : `Expires in ${hoursRemaining}h`}
        </Text>
      </View>

      {/* Badge grid */}
      <View flexDirection="row" flexWrap="wrap" gap="$2" paddingVertical="$2">
        {badges.map((badgeId) => {
